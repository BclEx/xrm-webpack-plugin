...
config.getRaw = function(prop) {
  if (prop) {
    ...
    return webpack.util.namespace.get(config.data, config.getPropString(prop));
  } else {
...
config.process = function(raw) {
  return webpack.util.recurse(raw, function(value) {
    ...
    return webpack.template.process(value, {data: config.data});
  });
};

// Set config data.
config.set = function(prop, value) {
  return webpack.util.namespace.set(config.data, config.getPropString(prop), value);
};

// Deep merge config data.
config.merge = function(obj) {
  webpack.util._.merge(config.data, obj);
  return config.data;
};

// Initialize config data.
config.init = function(obj) {
  webpack.log('Initializing config...ok');
...
config.requires = function() {
  var p = webpack.util.pluralize;
  var props = webpack.util._.toArray(arguments).map(config.getPropString);
  var msg = 'Verifying propert' + p(props.length, 'y/ies') +
    ' ' + webpack.log(props) + ' exist' + p(props.length, 's') +
    ' in config...';
  webpack.log(msg);
  ...
  if (config.data && failProps.length === 0) {
    ...
    webpack.log(msg);
    webpack.log('error: Unable to process task.');
    if (!config.data) {
      throw webpack.util.error('Unable to load config.');
    } else {
      throw webpack.util.error('Required config propert' +
